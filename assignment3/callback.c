...
void bubblesort(void* base, size_t num, size_t width, int (*comparator)(const void *, const void *)) {
    for (int n=num; n>1; --n){
        for (int i=0; i<n-1; i++) {
            //calculate current position of the two elements
            void* a = (void*)(((char*)base)+(i*width));
            void* b = (void*)(((char*)base)+((i+1)*width));
            if (comparator(a,b) > 0) {
                // swap those elements
                char temp[width]; // char serves as the type for "generic" byte arrays
                memcpy(temp, b,    width);
                memcpy(b,    a,    width);
                memcpy(a,    temp, width);
            }
...
{
    char* fname1 = ((struct student*)a)->fname;
    char* fname2 = ((struct student*)b)->fname;
...
{
    char* lname1 = ((struct student*)a)->lname;
    char* lname2 = ((struct student*)b)->lname;
...
int main()
{
    ...
    /*sort based on first name*/
    puts("Sorted by first name BUBBLESORT:");
    puts("-------------------------------");
    bubblesort(class, nstudents, sizeof(struct student), compare_first_name);
    apply(class, nstudents, printrec, NULL);
